...
description: Diagnostic audit of codebase health. Finds concrete problems with evidence - circular dependencies, dead code, coupling hotspots, oversized files, structural inconsistencies - and ranks them by severity. Use when asked to audit a codebase, find tech debt, check for dead code, or assess overall structural health. For deepening refactors (module interfaces, testability), use the architecture-improvement skill instead.
