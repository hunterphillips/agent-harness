...
## Docs mode (conditional)

Docs mode is ON when either holds; otherwise this is a pure interrogation — write nothing:

- The repo already maintains domain docs: `CONTEXT.md`, `CONTEXT-MAP.md`, or `docs/adr/` exists
- The user asks to capture decisions or terminology from the session
...
**Update CONTEXT.md inline.** When a term is resolved, update `CONTEXT.md` right there. Don't batch these up — capture them as they happen. Use the format in [CONTEXT-FORMAT.md](./CONTEXT-FORMAT.md).
