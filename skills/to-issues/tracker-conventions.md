...
Published issues are self-contained work items: a fresh session can pick one up and implement it directly, or feed it to `/create-plan` for a phased implementation plan. Write them durably — behavior and interfaces, not file paths — so they stay correct as the codebase moves.
